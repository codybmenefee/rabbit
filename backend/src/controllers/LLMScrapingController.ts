...
export class LLMScrapingController {
  ...
  public scrapeVideos = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const results = await this.scrapingService.scrapeVideos(videoIds);
      ...
      res.json({
        ...
        data: {
          ...
          summary: {
            ...
            averageCostPerVideo: metrics.costPerVideo,
    ...
    try {
      ...
      res.json({
        ...
        data: {
          videoCount,
          provider,
          model: model || (provider === 'anthropic' ? 'claude-3-haiku-20240307' : 'gpt-3.5-turbo'),
          estimates: {
            totalTokens,
