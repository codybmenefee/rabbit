 ...
 */
function parseVideoDataFast(html: string, videoId: string): ScrapedVideoData | null {
  ...
  try {
    ...
    if (ytDataMatch) {
      try {
        ...
        if (videoDetails) {
          if (videoDetails.title) data.title = sanitizeTitle(videoDetails.title);
    ...
    if (!data.title) {
      ...
      if (titleMatch) data.title = sanitizeTitle(titleMatch[1]);
    }
    
    if (!data.channelName) {
      const authorMatch = html.match(/<meta itemprop="author" content="([^"]+)"/);
      if (authorMatch) data.channelName = authorMatch[1];
    }
    ...
    if (!data.thumbnailUrl) {
      ...
      if (thumbMatch) data.thumbnailUrl = thumbMatch[1];
    }
 ...
 */
function parseVideoDataComplete(html: string, videoId: string): ScrapedVideoData | null {
  const data: ScrapedVideoData = {};
  ...
  try {
    ...
    if (!data.title) data.title = sanitizeTitle($('meta[property="og:title"]').attr('content') || '');
    if (!data.channelName) data.channelName = $('meta[itemprop="author"]').attr('content') || '';
    if (!data.description) data.description = $('meta[property="og:description"]').attr('content') || '';
    if (!data.thumbnailUrl) data.thumbnailUrl = $('meta[property="og:image"]').attr('content') || '';
      ...
      try {
        ...
        for (const obj of objects) {
          if (obj['@type'] === 'VideoObject') {
            if (!data.title && obj.name) data.title = sanitizeTitle(obj.name);
            if (!data.description && obj.description) data.description = obj.description;
            if (!data.duration && obj.duration) data.duration = parseDurationText(obj.duration);
            if (!data.publishedAt && obj.uploadDate) data.publishedAt = new Date(obj.uploadDate);
...
function parsePublishDate(dateText: string): Date | undefined {
  try {
...
if (parentPort) {
  parentPort.on('message', (task: WorkerTask) => {
    try {
      ...
      let result: ScrapedVideoData | null = null;
  ...
  module.exports = (task: WorkerTask): ScrapedVideoData | null => {
    if (task.enableFastParsing) {
